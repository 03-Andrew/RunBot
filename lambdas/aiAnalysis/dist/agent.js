...
exports.runNaturalLanguageAi = void 0;
...
const formatPace = (movingTimeSeconds, distanceMeters) => {
...
const summarizeRuns = (runs) => runs.map(summarizeRun);
const loadRunHistory = async (user, discordUserId, lookbackDays) => {
    ...
    const [recentActivities, storedActivities] = await Promise.all([
        (0, stravaApi_1.fetchStravaActivitiesSince)(user, afterUnixSeconds),
        (0, stravaApi_1.getStoredStravaActivitiesByDiscordId)(discordUserId),
    ]);
    return dedupeAndSortRuns([...recentActivities, ...storedActivities]);
};
const getLatestRun = async (context) => {
    if (!context.linkedStravaUser) {
        return {
            error: "No linked Strava account is available for this user.",
        };
    }
    const runs = await loadRunHistory(context.linkedStravaUser, context.discordUserId, DEFAULT_LOOKBACK_DAYS);
...
const getRecentRuns = async (context, args) => {
    if (!context.linkedStravaUser) {
        return {
            error: "No linked Strava account is available for this user.",
        };
    ...
    const runLimit = Number.isFinite(limit) && limit > 0 ? Math.min(Math.floor(limit), 15) : DEFAULT_RECENT_LIMIT;
    const safeLookbackDays = Number.isFinite(lookbackDays) && lookbackDays > 0
        ? Math.min(Math.floor(lookbackDays), 365)
        : DEFAULT_LOOKBACK_DAYS;
    const runs = await loadRunHistory(context.linkedStravaUser, context.discordUserId, safeLookbackDays);
    return {
...
const getWeeklyStats = async (context) => {
    if (!context.linkedStravaUser) {
        return {
            error: "No linked Strava account is available for this user.",
        };
    ...
    const activities = await (0, stravaApi_1.fetchStravaActivitiesSince)(context.linkedStravaUser, afterUnixSeconds);
    const stats = (0, stravaStats_1.calculateWeeklyStats)(activities);
    return {
        weeklyStats: {
...
const compareToPastRuns = async (context, args) => {
    if (!context.linkedStravaUser) {
        return {
            error: "No linked Strava account is available for this user.",
        };
    ...
    const safeLookbackDays = Number.isFinite(lookbackDays) && lookbackDays > 0
        ? Math.min(Math.floor(lookbackDays), 365)
        : DEFAULT_LOOKBACK_DAYS;
    const safeBaselineCount = Number.isFinite(baselineRunCount) && baselineRunCount > 0
    ...
    return {
        comparison: {
            ...
            baselineAveragePacePerKm: paceAverage != null ? formatPace(paceAverage, 1000) : undefined,
            paceDeltaSecondsPerKm: paceAverage != null && latestPaceSecondsPerKm != null
...
const TOOL_DECLARATIONS = [
    {
        name: "get_latest_run",
        description: "Get the user's most recent running activity and a small recent-run sample for context.",
        parameters: {
            type: "object",
            properties: {},
            required: [],
    ...
    },
    {
        ...
        description: "Get a list of recent running activities for comparison, pacing, and trend analysis.",
        parameters: {
            type: "object",
            properties: {
                limit: {
    ...
    },
    {
        ...
        description: "Get the user's weekly running volume, count, total time, longest run, and average pace.",
        parameters: {
            type: "object",
            properties: {},
            required: [],
    ...
    },
    {
        ...
        description: "Compare the user's latest run with a baseline of prior runs to estimate whether it was faster, slower, easier, or heavier than usual.",
        parameters: {
            type: "object",
            properties: {
                baselineRunCount: {
...
const getFunctionCalls = (content) => (content.parts ?? [])
    .map((part) => part.functionCall ?? part.function_call)
    .filter((call) => Boolean(call && call.name));
const getText = (content) => (content.parts ?? [])
    .map((part) => part.text)
    .filter((text) => typeof text === "string" && text.length > 0)
    .join("\n");
const executeTool = async (context, call) => {
...
const buildSystemInstruction = (hasLinkedStrava) => [
...
const callGemini = async (contents, hasLinkedStrava) => {
...
const runNaturalLanguageAi = async (prompt, discordUserId) => {
    const linkedStravaUser = await (0, stravaApi_1.getLinkedStravaUserByDiscordId)(discordUserId);
    const context = {
        discordUserId,
        linkedStravaUser,
    };
...
exports.runNaturalLanguageAi = runNaturalLanguageAi;
